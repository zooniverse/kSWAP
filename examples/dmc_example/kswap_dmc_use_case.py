...
class User(object):
  ...
  def update_confusion_matrix(self, gold_label, label):
    ...
    gold_label_class = self.classes[gold_label]
    mapped_label = self.label_map[gold_label_class][self.classes.index(label)]
    confusion_matrix['n_gold'][gold_label] += 1
    print(confusion_matrix['matrix'])
    print(confusion_matrix['matrix'][gold_label][mapped_label])
    confusion_matrix['matrix'][gold_label][mapped_label] += 1
    print(confusion_matrix, gold_label_class, label, mapped_label)
    print(confusion_matrix['matrix'][gold_label][mapped_label])
    print('##################')
...
class Subject(object):
  ...
  def update_score(self, label, user):
    ...
    #print(self.classes)
    denomenator = sum([user.user_score[c][label_index] * self.score[c] \
                       for c in self.classes])
                       
    for c in self.classes:
      #c_index = self.classes.index(c)
      numerator = (self.score[c] *  user.user_score[c][label_index])
...
class kSWAP(object):
  ...
  def process_classification(self, cl, online=False):
    ...
    if online:
      ...
      assert gold_label < len(self.config.classes)
